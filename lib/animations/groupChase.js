...
async function startGroupChase(groupId, lightIds, speed = 1000, bgColor = '#800080', headColor = '#0000ff') {
  ...
  timers.set(groupId, { timer, speed, head: 0, lightIds, bgColor, headColor });
}
...
function updateGroupChaseSpeed(groupId, newSpeed) {
  ...
  if (existing) {
    ...
    startGroupChase(groupId, existing.lightIds, newSpeed, existing.bgColor, existing.headColor);
  }
}
