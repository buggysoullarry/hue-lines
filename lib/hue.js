...
async function getRooms() {
  try {
    ...
    try {
      ...
      const devicesResponse = await axios.get(`${baseUrlV2}/resource/device`, {
        headers: { 'hue-application-key': HUE_APPKEY },
        httpsAgent: new https.Agent({ rejectUnauthorized: false }),
        timeout: 5000
      });
      ...
      lightsV2Response.data.data.forEach(light => {
        ...
        if (device) {
          const { product_name, archetype } = device.product_data;
          const isOmniGlow = product_name && product_name.toLowerCase().includes('omniglow') ||
                            (archetype === 'lightstrip' && light.gradient);
          const v1Id = light.id_v1.split('/').pop();
          isOmniGlowMap[v1Id] = isOmniGlow;
        }
      });
    } catch (v2Error) {
      console.warn('Hue API v2 not available, OmniGlow detection may be limited:', v2Error.message);
    }
      ...
      .map(([id, room]) => ({
        ...
        lights: room.lights.map(lightId => {
          ...
          return { 
            ...
            isOmniGlow: isOmniGlowMap[lightId] || false,
            color: hslToHex(light.state.hue / 65535, light.state.sat / 254, light.state.bri / 254)
          };
        })
      }));
...
// Placeholder: Get light from v2 API
async function getLightV2(lightId) {
  try {
    const response = await axios.get(`${baseUrlV2}/resource/light/${lightId}`, {
      headers: { 'hue-application-key': HUE_APPKEY },
      httpsAgent: new https.Agent({ rejectUnauthorized: false }),
      timeout: 5000
    });
    ...
    console.error(`Error fetching light ${lightId} from v2:`, error);
    throw error;
  }
}

module.exports = { getLights, getRooms, startPlay, stopPlay, setLightOn, setLightOff, setLightName, setLightBrightness, setLightColor, setLightGradient, getLightV2, hexToXy };