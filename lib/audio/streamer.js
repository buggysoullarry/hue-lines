...
class AudioStreamer extends EventEmitter {
  ...
  _saveVolume(vol) {
    try {
      ...
      log.warn(`Could not save volume: ${err.message}`);
    }
  }
  ...
  // Reset the AirPlay connection (flushes buffered audio)
  _resetSender() {
    if (this.sender) {
      try { this.sender.stop(); } catch (err) {
        log.warn(`AirPlay reset error: ${err.message}`);
      }
      this.sender = null;
      this._connected = false;
    }
  ...
  async streamFile(trackInfo) {
    ...
    // Always reconnect to flush any buffered audio from previous track
    this._resetSender();
    await this.connect();
    ...
    // or when _clearTrackTimer is called (skip/stop)
