...
// Debounce: ignore duplicate button fires within 1.5s (Tap Dial sends initial_press + short_release)
const lastButtonFire = new Map();
          ...
          for (const event of events) {
            ...
            for (const item of event.data) {
              if (item.type === 'button' && item.button?.last_event) {
                ...
                // Fire on initial_press or short_release, but debounce to prevent
                // double-firing on Tap Dial (which sends both for one press)
                if ((buttonEvent.event === 'initial_press' || buttonEvent.event === 'short_release') && onButtonPress) {
                  const now = Date.now();
                  const lastFire = lastButtonFire.get(item.id) || 0;
                  if (now - lastFire > DEBOUNCE_MS) {
                    lastButtonFire.set(item.id, now);
                    onButtonPress(buttonEvent);
                  } else {
                    log.info(`EventStream: Debounced duplicate for button ${item.id}`);
                  }
                }
              }
              ...
              if (item.type === 'relative_rotary' && item.relative_rotary?.last_event) {
                ...
                // Don't log every rotary tick — too noisy
